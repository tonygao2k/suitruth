/**
 * 🔍 SuiVision 站点适配模块
 * 负责在 suivision.xyz 上识别地址并注入 Badge
 */
...
const STYLE_ID = 'suitruth-styles-suivision';
...
export const removeBadges = () => {
  ...
  console.log(`🧹 [SuiVision] 已移除 ${badges.length} 个 Badge，重置处理状态`);
};
...
/**
 * 🔍 查找 SuiVision 页面上的地址元素
 */
const findAddressElements = () => {
  const elements = [];

  // SuiVision 可能使用不同的 URL 结构
  const selectors = [
    'a[href*="/account/0x"]',
    'a[href*="/address/0x"]',
    'a[href*="/object/0x"]',
...
export const scanAndInjectBadges = async () => {
  ...
  if (elements.length === 0) {
    console.log('🔍 [SuiVision] 未发现新的地址元素');
    return;
  }

  console.log(`🔍 [SuiVision] 发现 ${elements.length} 个地址元素`);
  ...
  if (addresses.length === 0) {
    console.log('🔍 [SuiVision] 未提取到有效地址');
    return;
  }

  console.log(`🔍 [SuiVision] 提取到 ${addresses.length} 个唯一地址`);
  ...
  try {
    ...
    console.log(`✅ [SuiVision] 已注入 ${injectedCount} 个 Badge`);
  } catch (error) {
    console.error('❌ [SuiVision] 批量获取画像失败:', error);
  }
};
