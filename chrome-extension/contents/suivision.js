...
export const injectStyles = () => {
  ...
  addressLinks.forEach((el) => {
    ...
    badge.innerText = '🛡️';
    badge.title = 'SuiVision - SuiTruth 正在保护中';
    badge.style.cssText = `
      margin-left: 4px;
      font-size: 12px;
      cursor: help;
    ...
    // 应用样式（绿色主题）
    el.style.setProperty('background-color', 'rgba(34, 197, 94, 0.2)', 'important');
  ...
  if (count > 0) {
    console.log(`✅ SuiVision: 成功注入 ${count} 个标记`);
  }
};
...
export const removeStyles = () => {
  ...
  if (badges.length > 0) {
    console.log(`🔴 SuiVision: 已移除 ${badges.length} 个标记`);
  }
};
