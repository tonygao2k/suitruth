...
export const injectStyles = () => {
  // 查找所有可能包含地址的元素（链接、代码块、文本）
  const elements = document.querySelectorAll('a, code, span, div');
  let count = 0;
  ...
  elements.forEach((el) => {
    ...
    // 检查文本内容或 href 是否包含 Sui 地址
    const text = el.innerText || el.textContent || '';
    const href = el.href || '';
    const combined = text + ' ' + href;

    if (!isSuiAddress(combined)) return;
    ...
    badge.innerText = '🛡️';
    badge.title = 'Polymedia - SuiTruth 正在保护中';
    badge.style.cssText = `
      margin-left: 4px;
      font-size: 12px;
      cursor: help;
    ...
    // 应用样式（黄色主题）
    el.style.setProperty('background-color', 'rgba(251, 191, 36, 0.2)', 'important');
  ...
  if (count > 0) {
    console.log(`✅ Polymedia: 成功注入 ${count} 个标记`);
  }
};
...
export const removeStyles = () => {
  ...
  if (badges.length > 0) {
    console.log(`🔴 Polymedia: 已移除 ${badges.length} 个标记`);
  }
};
